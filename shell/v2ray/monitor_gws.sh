...
do
    ...
    do
        ...
        if [ $? -gt 0 ]
        then
            echo "$gw 与vultr连接失败，调用API失败"
        else
            echo "$gw 调用API成功，开始执行监控程序"
            ...
            then
                ...
                then
                    ...
                    echo `date +"%Y-%m-%d %H:%M:%S"` "等待删除VPS"
                    sleep 30
                    echo `date +"%Y-%m-%d %H:%M:%S"` "删除vps成功"
...
#                            ip=`curl -H "$key" https://api.vultr.com/v1/server/list?SUBID=$INSTALL_SUBID | python -m json.tool | grep -E "\bmain_ip\b"  |awk -F '"' '{print$4}'`
                    ...
                    echo `date +"%Y-%m-%d %H:%M:%S"` "正在验证$VPS 分组是否存在,请稍后..."
                    count2=`ansible $LABEL -m ping|grep SUCCESS|wc -l`
                    if [ $count2 -gt 0 ]
                    then
                        echo `date +"%Y-%m-%d %H:%M:%S"` "验证成功$vps 分组存在！分组名为$LABEL"
                    else
                        echo `date +"%Y-%m-%d %H:%M:%S"` "$vps 分组不存在，请重新输入......"
                        exit 1
                    fi
                    ...
                    echo `date +"%Y-%m-%d %H:%M:%S"` "流量小于250G,进行下一次监控循环"
                fi
            fi
