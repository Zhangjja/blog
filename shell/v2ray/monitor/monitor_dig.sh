...
kbandusage(){
        ...
        echo "`date +"%Y-%m-%d %H:%M:%S"`" "v2ray服务器的宽带使用率为$BANDWIDTH 字节                                                                                            "
        if [ -z "$BANDWIDTH" ]
        then
...
#	    mail -s "`date +"%Y-%m-%d %H:%M:%S"` $local_ip的宽带使用率为$BANDUSAGE" 124173026@qq.com

            if [ $BANDUSAGE -gt 250 ]
            then
                echo "`date +"%Y-%m-%d %H:%M:%S"`" "触发暂停keepalived条件，keepalived服务将停用"
                service keepalived stop
#		echo "`date +"%Y-%m-%d %H:%M:%S"` $local_ip的宽带使用率为$BANDUSAGE 触发暂停keepalived条件，keepalived服务将停用"| mail -s "`date +"%Y-%m-%d %H:%M:%S"` $local_ip" 124173026@qq.com
...
then
	...
	echo "`date +"%Y-%m-%d %H:%M:%S"`" "解析时间为$QUERY_TIME"
	...
	then
	        echo "`date +"%Y-%m-%d %H:%M:%S"`" "解析超时，本地使用的v2ray服务器dns解析异常，停用keepalived服务,发送邮件通知"
	        service keepalived stop
